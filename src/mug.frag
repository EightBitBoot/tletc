...
void main() {
    ...
    if(gl_FrontFacing) {
        color = texture(u_materials[f_materialId].texSampler, f_vTexCoords);
    }
    else {
    ...
    vec3 ambientColor  = vec3(color) * u_materials[f_materialId].ka;
    vec3 diffuseColor  = vec3(color) * u_materials[f_materialId].kd * max(dot(normalVec, lightVec), 0);
    vec3 specularColor = vec3(color) * u_materials[f_materialId].ks * pow(max(dot(viewVec, reflectVec), 0), u_materials[f_materialId].se);
